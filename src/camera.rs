...
impl Plugin for CameraPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_system_set(SystemSet::on_update(States::InGame).with_system(follow_user).with_system(set_user_position));
    }
}
...
fn set_user_position(mut camera: Query<(&mut Transform, &mut FollowCam),(With<Camera>, Without<PlayerControl>)>, player: Query<&GlobalTransform, Added<PlayerControl>>) {
...
fn follow_user(mut camera: Query<(&mut Transform, &mut FollowCam), (With<Camera>, Without<PlayerControl>)>, player: Query<&GlobalTransform, With<PlayerControl>>, time: Res<Time>,) {
